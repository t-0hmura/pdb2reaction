...
2. **Configuration merge** – Defaults → CLI → YAML (`dft` block). YAML overrides take precedence over CLI flags. Charge/multiplicity inherit `.gjf` metadata when present. For non-`.gjf` inputs, `-q` is required unless `--ligand-charge` is provided; when `--ligand-charge` is set, the structure is treated as an enzyme–substrate complex and `extract.py`’s charge summary derives the total charge; explicit `-q` still overrides. When a `.gjf` template omits charge/spin, defaults are `0` and `1` respectively.
...
| `-q, --charge INT` | Total charge supplied to PySCF (`calc.charge`). Required unless the input is a `.gjf` template that already stores charge or `--ligand-charge` supplies it. Overrides `--ligand-charge` when both are set. | Required unless template/`--ligand-charge` |
...
_Functional/basis selection defaults to `wb97m-v/def2-tzvpd` but can be overridden on the CLI. Charge/spin inherit `.gjf` template metadata when present. For non-`.gjf` inputs, `-q` is required unless `--ligand-charge` is provided; when `--ligand-charge` is set, the input is treated as an enzyme–substrate complex and `extract.py`’s charge summary computes the total charge; explicit `-q` still overrides. When a `.gjf` template omits charge/spin, defaults are `0` and `1` respectively. Set them explicitly for non-default states._
