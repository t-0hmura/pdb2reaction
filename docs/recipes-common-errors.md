...
| `--hessian-calc-mode Analytical` raises RuntimeError when `--workers > 1` | Drop to `--workers 1` for analytical Hessians, or use FiniteDifference (the default) | {ref}`Installation / environment problems <installation-environment-problems>` |
