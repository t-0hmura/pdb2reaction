...
| IRC does not terminate | Standalone `irc`: reduce `--step-size`, increase `--max-cycles`. In `all`: `--irc-step-size` / `--irc-max-cycles`. Confirm a single imaginary mode | {ref}`Calculation / convergence problems <calculation-convergence-problems>` |
...
First checks:
- Confirm one imaginary mode under the selected criterion and inspect its displacement along the reaction coordinate. The default is a mass-weighted Hessian eigenvalue < −10⁻⁶ Hartree/(bohr²·amu), equivalent to a frequency below about −5.14 cm⁻¹. Mode selection and explicit flattening share this criterion; every negative sign is also counted separately. See [freq](freq.md) for the deprecated `freq.zero_cutoff_cm` override.
