...
| `--preopt/--no-preopt` | Pre-optimize active site model endpoints before MEP search. Standalone `scan`, `scan2d`, and `scan3d` default `--preopt` to `False` (pass `--preopt` to enable). | `True` |
