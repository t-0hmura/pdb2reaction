...
- Each scan stage ends with a bond-change check (`has_bond_change`) on the final relaxed geometry; the per-stage result is recorded in the scan log and, when `--out-json` is set, in the aggregate `result.json` (under its `stages` array) written to the scan out-dir. The recursive MEP refinement (`path-search`) consumes the scan endpoints unconditionally — it is gated by `--refine-path`, not by the scan-stage bond-change flag.
