...
This document provides a comprehensive reference for all YAML configuration options used across `pdb2reaction` subcommands. In `pdb2reaction all`, layered YAML is supported via `--config` and `--override-yaml` (`--args-yaml` is a legacy alias of `--override-yaml`) with precedence **defaults < config < CLI < override-yaml**.
