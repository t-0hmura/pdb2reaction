...
対処:
- **すべて** の構造を同じ前処理パイプライン（同じプロトン化ツール、同じ設定）で作り直します。
...
- **大規模系（1000原子以上）**: より小さなポケット（`--radius 2.5`）を抽出するか、マルチGPUセットアップで実行を検討
