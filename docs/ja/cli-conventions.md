...
**`all -c/--center` の例外:** 抽出処理が、標準残基、イオン、
`--ligand-charge/-l` を合算して基準となる総電荷を導出します。
明示的な `-q` はこの値を置き換えず、一致を検証する指定として扱われ、
不一致なら実行を中断します。どちらの CLI 電荷指定もない場合は
`calc.charge` を同様に照合します。`-l` を指定した場合、それは抽出への入力となり、
設定ファイルの `calc.charge` より優先されます。
