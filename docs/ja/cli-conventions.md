...
表示します。`-q` を省略した場合、設定ファイルの `calc.charge` も同じ
扱いで使用します。どちらの総電荷指定もなければ抽出値を使用します。
`-l` のマッピングは引き続き抽出への入力です。
...
| `all --scan-lists` | インライン3-tupleのみ |
| `scan2d` | `(i,j,low,high)` 軸を2本含む1リテラル/ファイル |
| `scan3d` | `(i,j,low,high)` 軸を3本含む1リテラル/ファイル |
...
|--------|------|-----------|
| `0` | 成功 | すべてのサブコマンド |
| `1` | 予期しない内部エラー、または意図された utility の部分完了 | すべてのサブコマンド、`bond-summary` |
| `2` | CLI usage・設定・入力エラー、ゼロ step、依存関係不足、command-specific processing failure | Click/parser layer、`opt`、`tsopt`、`path-search`、`dft`、utility commands |
