...
連続する分子構造（R → TS → P や多段中間体列）の間の共有結合変化を、元素固有の共有結合半径によって検出・報告します。入力 *N* 構造に対し *N* − 1 個のペア比較ブロック（A→B, B→C, …）を生成して標準出力へ表示します。ファイルは作成されません。反応経路上の連続構造でどの共有結合が形成・切断されたかを確認するときに使用します。IRC 端点ペアの妥当性検証、多段機構のスクリーニング、`all` の post-processing 結果を手動で検算する場合などです。対応フォーマットは **XYZ**、**PDB**、**GJF**（拡張子から自動検出）で、距離はオングストローム（Å）で報告されます。
