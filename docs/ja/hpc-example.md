...
```{warning}
UMA バックエンドを `workers > 1` で実行している状態で `hessian_calc_mode="Analytical"` を明示指定すると `RuntimeError` が送出されます。`workers = 1` に下げるか、デフォルトの `FiniteDifference` を使用してください。{ref}`ja-hessian-evaluation` を参照してください。ORB / MACE / AIMNet2 は `workers` / `workers_per_node` を受け付けないため、この規則は適用されません。
```
