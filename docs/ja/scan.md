...
- 電荷はテンプレートがあれば継承されます。`.gjf` 以外の入力では `-q/--charge` が必須ですが、`--ligand-charge` がある場合は例外（PDB 入力、または `--ref-pdb` 付きXYZ/GJF）。明示的な `-q` は常に優先されます。**多重度は省略時に常に `1` となり、テンプレートからは継承されません。**
