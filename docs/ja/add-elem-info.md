...
PDB の ATOM/HETATM レコードの元素記号列（77–78）を修復します。固定列の原子名と残基情報から元素を推定します。`all` の事前確認で空欄が見つかると、元素を推定できるすべての ATOM/HETATM レコードを再処理します。誤った非空欄記号を直す場合、または空欄のある入力を単独コマンドへ渡す前に、このユーティリティを明示実行してください。
...
- `all` の自動修復では、推定可能な非空欄の元素記号も再処理します。自動修復の条件は [all](all.md) を参照してください。
