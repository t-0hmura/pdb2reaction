...
PDB ファイルの ATOM/HETATM レコードの元素記号カラム（77–78）を修復します。`Bio.PDB.PDBParser` で再パースし、各原子の元素を原子名と残基コンテキストから推定して、`Bio.PDB.PDBIO` で再シリアライズします（修復対象は元素記号カラム 77–78）。PDB の元素カラム（77–78）が欠落・誤記で、`extract` / `opt` / `tsopt` などの下流サブコマンドが入力を受け付けないときに使用します。`all` はプリフライトで `add-elem-info` を自動呼び出しするため、手動実行は単独サブコマンドの前処理用です。
...
## 注記
- 構造は `Bio.PDB.PDBIO` で再シリアライズされます（列のインプレース編集ではありません）。ATOM/HETATM の座標・占有率・B 因子・altloc・挿入コードは round-trip しますが、非 ATOM レコード（HEADER/REMARK/CONECT/ANISOU）とレガシーの電荷列（79–80）は保持されません。
