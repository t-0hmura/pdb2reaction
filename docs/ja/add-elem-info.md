...
PDB ファイルの ATOM/HETATM レコードの元素記号列（77–78）を修復します。固定列の原子名と残基情報から元素を推定し、必要な元素列だけを置換します。それ以外の列とレコードは入力のまま保持されます。PDB の元素列が欠落・誤記で、`extract` / `opt` / `tsopt` などの下流サブコマンドが入力を受け付けないときに使用します。`all` は事前チェックで `add-elem-info` を自動的に呼び出すため、手動実行は単独サブコマンドの前処理用です。
