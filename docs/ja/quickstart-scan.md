...
- 各スキャン stage は最終リラックス構造に対する bond-change check（`has_bond_change`）で終了し、stage ごとの結果は scan ログと、`--out-json` 指定時には scan 出力ディレクトリに書き出される集約 `result.json`（その `stages` 配列内）に記録されます。再帰的 MEP refinement（`path-search`）は scan の端点を**無条件**に消費し、ゲートは scan stage の bond-change フラグではなく `--refine-path` です。
