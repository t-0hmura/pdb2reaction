...
| **反応セグメント** | Reactive Segment | 反応候補のセグメント。要求した TS 最適化は通常の MEP／TS 候補を処理し、端点の結合変化は別に記録します |
...
| **Dimer** | Dimer Method | 完全な Hessian を計算せずに最低曲率モードを推定する TS 最適化法。`tsopt --opt-mode grad` で使用。pdb2reaction は Hessian Guided Dimer 変種を使い、活性部分空間の正確な Hessian を周期的に評価してダイマー方向を更新します |
| **Bofill** | Bofill Update | SR1（対称ランク 1）と PSB（Powell-symmetric-Broyden）を混合した Hessian 更新スキーム。鞍点探索に適します。`hessian_update: bofill` で選択され、RS-I-RFO と Dimer のフラット化ループで使用されます |
...
| **解析 Hessian** | Analytical Hessian | 選択した MLIP エネルギーを自動微分し、有限変位の打切り誤差を避ける（浮動小数点・autograd の挙動は残る）。速度と accelerator memory は backend/model/系に依存。`--hessian-calc-mode Analytical` で選択 |
...
| **Hessian（Hessian 行列）** | — | エネルギーの二階微分行列。振動解析では、質量重み付き Hessian から振動数と振動モードを求めます。振動解析や TS 最適化に使用します |
...
| **虚振動数** | Hessian 行列の負の固有値に対応する振動数。TS では 1 本のみ存在（一次鞍点）。負の cm⁻¹ 値で報告され、鞍点次数は選択した虚振動の分類基準で記述し、すべての負符号も別の診断値として保持します。 |

(ja-frequency-thresholds)=
### 虚振動の分類基準と QRRHO rotor cutoff

虚振動の分類基準と QRRHO rotor cutoff は用途が異なります。
