...
`sp` はデフォルトで出力を `result_sp/` 以下に書き出します。スカラーのエネルギーと `|force|_max` は stdout に出力され、`forces.npy`（`--hess` 指定時は `hessian.npy` も）は常にそこへ書き出されます。

| ファイル | 内容 | 書き出し |
|---|---|---|
| _stdout_ | スカラーのエネルギー (a.u.) と `|force|_max`。`[sp] energy = …` の形式で出力 | 常に |
| `forces.npy` | 原子単位 (Hartree / Bohr) の `(N, 3)` 力配列 | 常に |
| `hessian.npy` | 質量非加重の `(3N, 3N)` Hessian (Hartree / Bohr²) | `--hess` 指定時のみ |
