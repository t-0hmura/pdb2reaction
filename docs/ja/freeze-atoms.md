...
- **`path-opt --mep-mode dmf` と `path-search --mep-mode dmf`（soft restraint）:** ゼロ化の代わりに `HarmonicFixAtoms`（デフォルト `k_fix = 300 eV/Å²`、ASE 単位系）を各 image に追加し、凍結原子を調和拘束で relax させます（hard 拘束ではなく、初期座標から微小ずれが生じ得ます）。
