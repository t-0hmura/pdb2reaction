...
`fix-altloc` は、PDB ファイルから代替位置（altLoc）指示子を削除します。各原子について占有率（occupancy）に基づいて最良のコンフォーマーを選択し、重複を除去します。各原子について占有率最高のものを優先選択（同点はファイル内出現順）し、保持側の列 17 をスペース 1 文字に置換します。PDB に altLoc 指示子が残っていて、下流のパイプライン段階（`extract` / `opt` / `tsopt` など）がマルチコンフォーマー入力を受け付けない場合に使用します。クラスター抽出や構造最適化など、マルチコンフォーマー入力を受け付けない段階の前に実行してください。
