...
- `workers > 1` の場合、解析ヘシアンは無効化されます — `workers > 1` で `hessian_calc_mode: Analytical` を明示指定すると calculator 入口で `RuntimeError` が送出されます。デフォルトの `FiniteDifference` を使うか、`workers = 1` に下げてください。詳細は {ref}`MLIP Calculator のヘシアン評価モード <ja-hessian-evaluation>` を参照してください。
