...
- `workers > 1` の場合、解析ヘシアンは無効化されます — `workers > 1` では `hessian_calc_mode: Analytical` を明示指定していても解析ヘシアンは警告なく有限差分へダウングレードされます（ログ出力なし）。デフォルトの `FiniteDifference` を使うか、解析ヘシアンが必要な場合は `workers = 1` に下げてください。詳細は {ref}`MLIP Calculator のヘシアン評価モード <ja-hessian-evaluation>` を参照してください。
