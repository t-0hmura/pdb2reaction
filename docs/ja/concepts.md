...
```{important}
遷移状態（一次鞍点: First-Order Saddle Point）: Highest-Energy Image (HEI) や `tsopt` の出力は **TS 候補** として扱い、`irc`（両端が意図した極小構造へ到達すること）で検証してから解釈してください。`tsopt` は内部で虚振動数チェックを実施済みです — 出力に虚振動数が 1 本（|ν| >= 100 cm⁻¹）あることを確認してください。虚振動数が複数残る場合は `--flatten` の適用を検討してください。
```
...
```{important}
- ブール値オプションは `--flag` / `--no-flag` と `--flag True/False`（`yes/no`, `1/0` 含む）の両方を受け付けます。新規スクリプトでは `--flag`/`--no-flag` 形式を推奨します。
