...
- **`path-search` / `path-opt` はエンジンの例外です。** スタンドアロンで実行すると、`path-search` 自体が成果物となります（独自の `summary.log`、`mep.pdb`、`mep_trj.xyz`、`mep_plot.png`、`energy_diagram_MEP.png` を持つ `result_path_search/`）。`all` の内部では、その生の出力は `_work/path_opt/`（`--refine-path True` 指定時は `_work/path_search/`）下のスクラッチとして扱われ、マージされた成果物（`mep.pdb`、`mep_trj.xyz`、`mep_w_ref.pdb`、`energy_diagram_MEP.png`）のみがパイプラインのルートに昇格されます。
