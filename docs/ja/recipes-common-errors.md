...
| IRC が正常に終了しない | 単独の `irc`: `--step-size` / `--max-cycles`。`all`: `--irc-step-size` / `--irc-max-cycles`。開始構造の判定対象の虚振動が 1 本か確認 | {ref}`計算 / 収束の問題 <ts-calc-conv>` |
...
- 最初の確認:
 - 選択した分類基準による虚振動が1本で、対応するモードが反応座標方向の変位を示すか。既定の基準は質量重み付き Hessian の固有値 < −10⁻⁶ Hartree/(bohr²·amu)（振動数では約 −5.14 cm⁻¹ 未満）です。分類基準はモード選択・明示的な平坦化に共通で、負符号の総数は別途記録します。非推奨の `freq.zero_cutoff_cm` 上書きは [freq](freq.md) を参照してください。
