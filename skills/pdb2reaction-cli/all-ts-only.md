...
| `post["ts_imag"]["n_imag"] >= 2` | Two near-degenerate negative modes | Normal for some metalloenzyme TSs; check whether the second imaginary mode is a residual translation / rotation mode (often resolved by tightening `freeze_atoms`) |
| `irc.bond_changes == {}` (no bonds change) | TS connects two essentially identical wells | Verify the imaginary mode visualization in `freq/`; this is sometimes a non-physical TS |
