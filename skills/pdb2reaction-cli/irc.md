...
internally and post-processes the endpoints). Pass `--opt-mode-post grad` to
`all` only when you deliberately want L-BFGS endpoint refinement (it is a CLI flag, not a YAML key).
