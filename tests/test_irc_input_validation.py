...
@pytest.mark.parametrize(
    "setting",
    ["never_stop", "forward", "backward", "force_inflection", "check_bonds"],
)
def test_irc_yaml_boolean_keys_reject_quoted_strings(
    tmp_path: Path, setting: str,
) -> None:
    source = tmp_path / "atom.xyz"
    source.write_text("1\natom\nHe 0 0 0\n", encoding="utf-8")
    config = tmp_path / "config.yaml"
    config.write_text(f'irc:\n  {setting}: "false"\n', encoding="utf-8")

    result = CliRunner().invoke(
        root_cli,
        [
            "irc", "-i", str(source), "-q", "0", "--dry-run",
            "--config", str(config),
        ],
    )

    assert result.exit_code == 2
    assert f"irc.{setting} must be a YAML boolean" in result.output
