        ...
        if name in single_negative_to_positive:
            ...
            if explicit_literal:
                if parsed is None:
                    ...
                    legacy_used = True
                    if not parsed:
                        # `--no-FLAG False` (legacy "keep FLAG on")
                        # -> emit the positive name Click registered.
                        normalized.append(positive_name)
                    # `--no-FLAG True` (legacy "turn FLAG off") is a no-op
                    # for single-flag bools: emitting the synthetic
                    # negative name would trigger "no such option" since
                    # Click only knows the positive form.
                i = next_i
                continue
