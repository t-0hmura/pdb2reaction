...
def _write_mode_trj_and_pdb(geom,
    ...
    """
    Write a single mode animation as .trj (XYZ-like) and optionally PDB/mmCIF.

    If `ref_pdb` is provided and is a structure file, the structure output is generated by
    converting the .trj using the input structure as the template; on failure, an ASE fallback is used.
    Set `write_pdb=False` to skip structure generation.
    """
    ...
    if write_pdb and ref_pdb is not None and is_structure_path(ref_pdb):
    ...
    needs_structure = write_pdb and out_pdb is not None

    if not needs_structure:
        return

    ref_for_conv = ref_pdb if (ref_pdb and is_structure_path(ref_pdb)) else None
    try:
        convert_xyz_like_outputs(
            out_trj,
...
def cli(
    ...
    try:
        ...
        # Reference structure (only when input is PDB/mmCIF)
        ...
        for k, idx in enumerate(order[:n_write], start=1):
            ...
            _write_mode_trj_and_pdb(
                ...
                out_pdb=(
                    out_dir_path / f"mode_{k:04d}_{freq:+.2f}cm-1{structure_ext}"
                    if write_pdb and structure_ext is not None
                    else None
                ),
            )
